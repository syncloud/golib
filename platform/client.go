...
func (c *Client) GetAppStorageDir(app string) (string, error) {
	c.logger.Info("get app storage dir", zap.String("app", app))
	resp, err := c.client.Get(fmt.Sprintf("http://unix/app/storage_dir?name=%s", app))
	if err != nil {
		return "", err
	}
	if resp.StatusCode != 200 {
		return "", fmt.Errorf("get app storage dir, %s", resp.Status)
	}
	bodyBytes, err := io.ReadAll(resp.Body)
	if err != nil {
		return "", err
